...
# Function to check if AI reporting should be disabled (checked dynamically)
def _is_ai_disabled():
    return os.getenv('DISABLE_AI_REPORTING', 'false').lower() == 'true' or '--no-ai' in sys.argv

# For backward compatibility, keep DISABLE_AI as a function call
DISABLE_AI = _is_ai_disabled()
...
def _colorize_text(text: str) -> str:
    ...
    bullish_keywords = [
        'Bullish', 'UP', 'up', 'Rising', 'Overbought', 'Excellent', 'Good',
    ...
    bearish_keywords = [
        ...
        'capitulation', 'drawdown', 'vulnerable', 'losses', 
        # 'risk'
    ]
...
def _get_latest_volatility_metrics(volatility_path: str) -> dict:
    ...
    try:
        ...
        latest = df.iloc[-1]
        return {
            'sharpe': latest.get('sharpe_ratio_60d', None),  # Use 60-day Sharpe for better stability
            'sharpe_30d': latest.get('sharpe_ratio_30d', None),
            'sharpe_60d': latest.get('sharpe_ratio_60d', None),
            'sharpe_90d': latest.get('sharpe_ratio_90d', None),
...
def _get_latest_technical_metrics(symbol: str) -> dict:
    """Reads the technical analysis file and returns the latest metrics."""
    technical_path = f"data/analysis/technical_analysis_{symbol}.csv"
    if not os.path.exists(technical_path):
        return {}
    try:
        df = pd.read_csv(technical_path, parse_dates=['date'])
        if df.empty:
            return {}
        latest = df.iloc[-1]
        return {
        ...
        print(f"Could not read or process technical file {technical_path}: {e}")
        return {}
...
def _get_volatility_interpretation(metrics: dict) -> str:
    ...
    # Sharpe Ratio interpretation (using 60d for stability)
    if metrics.get('sharpe') is not None:
        sharpe = metrics['sharpe']
        if sharpe > 2:
            interpretations.append("**Sharpe Ratio (60d)**: Excellent risk-adjusted returns")
        elif sharpe > 1:
            interpretations.append("**Sharpe Ratio (60d)**: Good risk-adjusted returns")
        else:
            interpretations.append("**Sharpe Ratio (60d)**: Poor risk-adjusted returns")
...
def _build_market_overview_section() -> list[str]:
    """
    Builds the brief market overview section for the front page.
    """
    overview_section = []
    
    try:
        print("  - Generating brief market overview...")
        
        if _is_ai_disabled():
            print("  - AI reporting disabled, using placeholder text")
            overview_section.append("\n## Market Overview\n\n")
            overview_section.append("*Market overview temporarily unavailable (AI disabled)*\n\n")
        ...
        if market_summary.get('success'):
            overview_section.append(market_summary['analysis'])
            overview_section.append("\n\n")
        else:
            overview_section.append("*Market overview temporarily unavailable*\n\n")
...
def generate_report():
    ...
    summary_table = [
        ...
        "*Signal Direction: 🚀 = Bull Trend, 📈 = Bull Bias, 🔻 = Bear Trend, 📉 = Bear Bias, ➿ = Mixed*\n\n",
        "*Market Activity: ✅ = Active market, 💤 = Quiet market*\n\n",
        # "*PDF Legend: [STRONG] = Strong Preserved, [WARNING] = Strong Compressed, [WEAK] = Weak Compressed*\n\n",
        "| Asset   | Market Phase | Days in Phase | Signal Detail | Direction / Activity | Vol% | Liquidations | Sharpe (60d) |\n",
        "|:--------|:---------------|:----:|:--------------|:---------|:----:|:------------|:-------------|\n"
    ]
    ...
    for symbol in assets:
        ...
        # Get signal detail components
        signal_detail = _get_signal_detail(signals_path)
        
        summary_table.append(f"| {symbol} | {_colorize_text(regime)} | {persistence} | {signal_detail} | {signal_strength_indicator} | {vol_pct} | {_colorize_text(instability)} | {sharpe} |\n")
    ...
    market_overview_section = _build_market_overview_section()
    ...
    report_content = [report_title] + summary_table + market_overview_section + detailed_sections
...
""")
    ...
    try:
        ...
        try:
            # Create PDF-friendly version by replacing emojis with text symbols
            pdf_friendly_content = _convert_emojis_for_pdf(final_markdown_content)
            html_content = markdown.markdown(pdf_friendly_content, extensions=['tables'])
            ...
            # Set PDFKit options for better Unicode and emoji support
            options = {
                'encoding': "UTF-8",
                'no-outline': None,
...
def _generate_asset_section(symbol: str, signals_path: str, instability: str, volatility_path: str) -> str | None:
    ...
    try:
        ...
        # Get technical metrics for consistent RSI
        technical_metrics = _get_latest_technical_metrics(symbol)
        ...
        # --- Build Enhanced Signal Table with Compression Context ---
        last_5_days = df.tail(5)
        
        # Check if we have new compression data
        has_compression_data = 'original_state' in df.columns and 'realized_vol_10d_percentile' in df.columns
        
        if has_compression_data:
            table_md = "| Date | Close | Vol-Adjusted Signal | Direction Signal | Vol% | Price | OI | CVD |\n"
            table_md += "|:---|---:|:---|:---|---:|:---|:---|:---|\n"
        else:
        ...
        for _, row in last_5_days.iterrows():
            ...
            if has_compression_data:
                # Enhanced table with compression insights
                final_signal = _colorize_text(row['state'])
                original_signal = _colorize_text(row.get('original_state', 'N/A'))
                vol_pct = f"{row.get('realized_vol_10d_percentile', 0):.0f}%"
                
                # Add compression indicator -- does this need to be fixed?
                if row['state'] == 'Compression' and row.get('original_state') != 'Compression':
                    final_signal = f"{final_signal}"
                    if row.get('original_state') in ['Bullish Trending', 'Bearish Trending']:
                        original_signal = f"{original_signal}"  # Strong signal was compressed
                elif row['state'] in ['Bullish Trending', 'Bearish Trending']:
                    final_signal = f"{final_signal}"  # Strong signal preserved
                ...
                table_md += f"| {date_str} | {close_str} | {final_signal} | {original_signal} | {vol_pct} | {price_state_colored} | {oi_state_colored} | {cvd_state_colored} |\n"
            else:
        ...
        signal_strength_section = ""
        if 'original_state' in df.columns and 'realized_vol_10d_percentile' in df.columns:
            latest_vol = latest.get('realized_vol_10d_percentile', 0)
            original_state = latest.get('original_state', latest['state'])
            
            # Define signal direction/alignment categories
            bull_trend_signals = ['Bullish Trending']
            bull_bias_signals = ['Bullish Exhaustion', 'Contrarian Bullish']
            bear_trend_signals = ['Bearish Trending']
            bear_bias_signals = ['Bearish Exhaustion', 'Contrarian Bearish']
            mixed_signals = ['Bullish Disagreement', 'Bearish Disagreement', 'Noise (Bullish Bias)', 'Noise (Bearish Bias)']
            
            # Determine market tone (Active vs Quiet) based on actual volatility
            is_active_market = latest_vol >= 20  # Threshold for "active" market
            market_tone = "✅" if is_active_market else "💤"
            
            if latest['state'] == 'Compression' and original_state != 'Compression':
                # Signal was suppressed due to quiet market, but show current market tone
                if original_state in bull_trend_signals:
                    signal_strength_section = f"**Signal Strength (🚀 {market_tone}):** Fully aligned up-trend in {'active' if is_active_market else 'quiet'} market ({latest_vol:.0f}% vol). Watch-list rather than immediate trade.\n\n"
                elif original_state in bull_bias_signals:
                    signal_strength_section = f"**Signal Strength (📈 {market_tone}):** Partial bullish alignment in {'active' if is_active_market else 'quiet'} market ({latest_vol:.0f}% vol). Wait for volatility expansion.\n\n"
                elif original_state in bear_trend_signals:
                    signal_strength_section = f"**Signal Strength (🔻 {market_tone}):** Fully aligned down-trend in {'active' if is_active_market else 'quiet'} market ({latest_vol:.0f}% vol). Watch-list rather than immediate trade.\n\n"
                elif original_state in bear_bias_signals:
                    signal_strength_section = f"**Signal Strength (📉 {market_tone}):** Partial bearish alignment in {'active' if is_active_market else 'quiet'} market ({latest_vol:.0f}% vol). Wait for volatility expansion.\n\n"
                else:
                    signal_strength_section = f"**Signal Strength (➿ {market_tone}):** No clear alignment in {'active' if is_active_market else 'quiet'} market ({latest_vol:.0f}% vol). Ignore until volatility expands.\n\n"
            elif latest['state'] in bull_trend_signals:
                # Strong bull trend in active market
                signal_strength_section = f"**Signal Strength (🚀 {market_tone}):** Fully aligned up-trend in {'active' if is_active_market else 'quiet'} market ({latest_vol:.0f}% vol). High-conviction long bias.\n\n"
            elif latest['state'] in bull_bias_signals:
                # Bull bias in active market
                signal_strength_section = f"**Signal Strength (📈 {market_tone}):** Partial bullish alignment in {'active' if is_active_market else 'quiet'} market ({latest_vol:.0f}% vol). Possible early reversal signal.\n\n"
            elif latest['state'] in bear_trend_signals:
                # Strong bear trend in active market
                signal_strength_section = f"**Signal Strength (🔻 {market_tone}):** Fully aligned down-trend in {'active' if is_active_market else 'quiet'} market ({latest_vol:.0f}% vol). High-conviction short bias.\n\n"
            elif latest['state'] in bear_bias_signals:
                # Bear bias in active market
                signal_strength_section = f"**Signal Strength (📉 {market_tone}):** Partial bearish alignment in {'active' if is_active_market else 'quiet'} market ({latest_vol:.0f}% vol). Possible early reversal signal.\n\n"
            else:
                # Mixed signals
                signal_strength_section = f"**Signal Strength (➿ {market_tone}):** No clear alignment in {'active' if is_active_market else 'quiet'} market ({latest_vol:.0f}% vol). Sideways chop.\n\n"
        ...
        # Add combined plot
        combined_plot_path = os.path.join(project_root, 'plots', 'svg', f'combined_plot_{symbol}.svg')
        if os.path.exists(combined_plot_path):
            abs_path = os.path.abspath(combined_plot_path)
            section.append(f'\n<img src="{abs_path}" style="width: 70%; height: auto; display: block; margin-left: auto; margin-right: auto; image-rendering: auto; shape-rendering: geometricPrecision;" alt="Combined Plot for {symbol}">\n')
        ...
        # Add technical plot
        technical_plot_path = os.path.join(project_root, 'plots', 'svg', f'technical_plot_{symbol}.svg')
        if os.path.exists(technical_plot_path):
            abs_path = os.path.abspath(technical_plot_path)
            section.append(f'\n<img src="{abs_path}" style="width: 100%; height: auto; display: block; margin-left: auto; margin-right: auto; image-rendering: auto; shape-rendering: geometricPrecision;" alt="Technical Plot for {symbol}">\n')

        # --- Add Bitcoin Brief AI Opinion (if this is Bitcoin) ---
        if symbol == "BTCUSDT" and not _is_ai_disabled():
            try:
                print(f"    - Generating brief Bitcoin AI opinion...")
                bitcoin_brief = generate_bitcoin_brief_opinion_alt()
                if bitcoin_brief.get('success'):
                    section.append("\n### AI Market Insight\n\n")
                    section.append(bitcoin_brief['analysis'])
                    section.append("\n\n")
            except Exception as e:
                print(f"    - Warning: Bitcoin brief AI generation failed: {e}")
        ...
        # --- Add Technical AI Perspective (if this is Bitcoin) ---
        if symbol == "BTCUSDT" and not _is_ai_disabled():
            try:
                print(f"    - Generating technical AI perspective...")
                tech_ai = generate_technical_interpretation_opinion_alt()
                if tech_ai.get('success'):
                    section.append("\n### AI Technical Perspective\n\n")
                    section.append(tech_ai['analysis'])
                    section.append("\n\n")
            except Exception as e:
                print(f"    - Warning: Technical AI generation failed: {e}")
        # ------------------------------------
        ...
        if volatility_metrics:
            ...
            # Use technical analysis RSI for consistency with Technical Interpretation section
            technical_rsi = technical_metrics.get('rsi') if technical_metrics else None
            if technical_rsi is not None:
                # Determine RSI state based on technical analysis RSI
                if technical_rsi > 70:
                    rsi_state = "Overbought"
                elif technical_rsi < 30:
                    rsi_state = "Oversold"
                else:
                    rsi_state = "Neutral"
                section.append(f"| RSI | {technical_rsi:.1f} ({_colorize_text(rsi_state)}) |\n")
            elif volatility_metrics.get('rsi') is not None:
                # Fallback to volatility RSI if technical analysis not available
            ...
            if volatility_metrics.get('sharpe') is not None:
                section.append(f"| Sharpe Ratio (60d) | {volatility_metrics['sharpe']:.2f} |\n")
...
def _get_signal_strength_summary(signals_path: str) -> tuple[str, str]:
    """
    Returns a signal strength indicator and volatility percentage for the summary table.
    Returns: (signal_strength_emoji, volatility_percentage)
    """
    try:
        if not os.path.exists(signals_path):
            return "❓", "N/A"
        ...
        # Define signal direction/alignment categories (consistent with detailed section)
        bull_trend_signals = ['Bullish Trending']
        bull_bias_signals = ['Bullish Exhaustion', 'Contrarian Bullish']
        bear_trend_signals = ['Bearish Trending']
        bear_bias_signals = ['Bearish Exhaustion', 'Contrarian Bearish']
        ...
        # Check if we have compression data
        if 'original_state' in df.columns and 'realized_vol_10d_percentile' in df.columns:
            vol_pct = f"{latest.get('realized_vol_10d_percentile', 0):.0f}%"
            original_state = latest.get('original_state', latest['state'])
            latest_vol = latest.get('realized_vol_10d_percentile', 0)
            
            # Determine market tone based on actual volatility
            is_active_market = latest_vol >= 20
            market_tone = "✅" if is_active_market else "💤"
            
            if latest['state'] == 'Compression' and original_state != 'Compression':
                # Signal was suppressed due to quiet market, but show current market tone
                if original_state in bull_trend_signals:
                    return f"🚀 {market_tone}", vol_pct  # Bull Trend with current market tone
                elif original_state in bull_bias_signals:
                    return f"📈 {market_tone}", vol_pct  # Bull Bias with current market tone
                elif original_state in bear_trend_signals:
                    return f"🔻 {market_tone}", vol_pct  # Bear Trend with current market tone
                elif original_state in bear_bias_signals:
                    return f"📉 {market_tone}", vol_pct  # Bear Bias with current market tone
                else:
                    return f"➿ {market_tone}", vol_pct  # Mixed with current market tone
            elif latest['state'] in bull_trend_signals:
                return f"🚀 {market_tone}", vol_pct  # Bull Trend with proper market tone
            elif latest['state'] in bull_bias_signals:
                return f"📈 {market_tone}", vol_pct  # Bull Bias with proper market tone
            elif latest['state'] in bear_trend_signals:
                return f"🔻 {market_tone}", vol_pct  # Bear Trend with proper market tone
            elif latest['state'] in bear_bias_signals:
                return f"📉 {market_tone}", vol_pct  # Bear Bias with proper market tone
            else:
                return f"➿ {market_tone}", vol_pct  # Mixed signal with proper market tone
        else:
            # Fallback for old format
            if latest['state'] in bull_trend_signals:
                return "🚀", "N/A"
            elif latest['state'] in bull_bias_signals:
                return "📈", "N/A"
            elif latest['state'] in bear_trend_signals:
                ...
                return "📉", "N/A"
            elif latest['state'] == 'Compression':
                return "💤", "N/A"
            else:
                return "➿", "N/A"
                
    except Exception as e:
        print(f"Error getting signal strength for {signals_path}: {e}")
        return "❓", "N/A"

def _get_signal_detail(signals_path: str) -> str:
    """
    Returns a formatted string showing the individual Price/OI/CVD components.
    Returns: "P↑ OI↑ CVD↑" format to show signal alignment
    """
    try:
        if not os.path.exists(signals_path):
            return "N/A"
        ...
        price_state = latest.get('price_state', 'N/A')
        oi_state = latest.get('oi_state', 'N/A')
        cvd_state = latest.get('cvd_state', 'N/A')
        
        # Convert to arrows
        def state_to_arrow(state):
            if state == 'UP':
                return '↑'
            elif state == 'DOWN':
                return '↓'
        ...
        p_arrow = state_to_arrow(price_state)
        oi_arrow = state_to_arrow(oi_state)
        cvd_arrow = state_to_arrow(cvd_state)
        
        return f"P{p_arrow} OI{oi_arrow} CVD{cvd_arrow}"
        
    except Exception as e:
        print(f"    - Error getting signal detail for {signals_path}: {e}")
        return "N/A"

def _convert_emojis_for_pdf(markdown_content: str) -> str:
    """
    Converts emojis to PDF-friendly text symbols for better rendering in wkhtmltopdf.
    This ensures emojis show up properly in the PDF version.
    """
    # Define emoji to text replacements
    emoji_replacements = {
        # Signal direction/alignment emojis
        '🚀': '[BULL TREND]',
        '📈': '[BULL BIAS]',
        '🔻': '[BEAR TREND]',
        '📉': '[BEAR BIAS]',
        ...
        # Legacy emojis (for backward compatibility)
        '😴': '[COMPRESSION]',
        '💪': '[POWER]',
        '❓': '[UNKNOWN]',
        
        # Arrow emojis (backup in case Unicode arrows fail)
        #'↑': '^',
        #'↓': 'v',
        
        # Other emojis that might appear
        '⭐': '[STAR]',
        '🔥': '[FIRE]',
        '💎': '[DIAMOND]',
        '🎯': '[TARGET]',
...
def generate_bitcoin_only_report():
    """
    Generates a Bitcoin-only report with the same structure as the full report.
    """
    print("Generating Bitcoin-only market analysis report...")
    os.makedirs(REPORTS_DIR, exist_ok=True)
    
    report_date = date.today()
    report_title = f"# Bitcoin Market Analysis - {report_date.isoformat()}\n\n"
    
    # Focus only on Bitcoin
    assets = ["BTCUSDT"]
    
    # 2. Build Market Summary Table (Bitcoin only)
    summary_table = [
        "## Bitcoin Market Summary\n\n",
        "*The following analysis focuses exclusively on Bitcoin market conditions.*\n\n",
        "*Signal Direction: 🚀 = Bull Trend, 📈 = Bull Bias, 🔻 = Bear Trend, 📉 = Bear Bias, ➿ = Mixed*\n\n",
        "*Market Activity: ✅ = Active market, 💤 = Quiet market*\n\n",
        "| Asset   | Market Phase | Days in Phase | Signal Detail | Direction / Activity | Vol% | Liquidations | Sharpe (60d) |\n",
        "|:--------|:---------------|:----:|:--------------|:---------|:----:|:------------|:-------------|\n"
    ...
    # --- Build Bitcoin-focused Market Overview Section ---
    market_overview_section = _build_bitcoin_market_overview_section()

    # 3. Build Detailed Analysis Section (Bitcoin only)
    detailed_sections = [
        '\n<div style="page-break-before: always;"></div>\n\n',
    ]
    
    for symbol, details in asset_details_map.items():
        if not os.path.exists(details['signals_path']):
            print(f"    - Warning: Signal file not found at {details['signals_path']}. Skipping.")
            continue
    ...
    # 4. Build AI Analysis Section (if enabled)
    ai_analysis_section = []
    if not _is_ai_disabled():
        ai_section = _build_ai_analysis_section()
        if ai_section:
            ai_analysis_section.append('\n<div style="page-break-before: always;"></div>\n\n')
            ai_analysis_section.extend(ai_section)

    # 5. Build Appendix (same approach as main report)
    appendix = _get_rules_appendix()

    # 6. Combine all sections
    full_report = [report_title] + summary_table + market_overview_section + detailed_sections + ai_analysis_section
    
    # Add appendices inline (same as main report)
    full_report.append('\n<div style="page-break-before: always;"></div>\n\n')
    full_report.append("# Appendix: Rule Definitions\n\n")
    full_report.append(appendix)
    
    # Add volatility metrics appendix
    full_report.append('\n<div style="page-break-before: always;"></div>\n\n')
    full_report.append("# Appendix: Volatility Metrics\n\n")
    full_report.append("""
### Risk Metrics Glossary
...
- **1-2**: Good downside-adjusted returns
- **0-1**: Moderate downside-adjusted returns
- **< 0**: Poor downside-adjusted returns
...
- **30 days**: Short-term view
- **60 days**: Medium-term view (primary for Sharpe ratio)
- **90 days**: Long-term view
- **120 days**: Extended view (for Max Drawdown)
""")

    # 7. Write to file
    report_filename = f"bitcoin_report_{report_date.strftime('%Y%m%d')}.md"
    report_path = os.path.join(REPORTS_DIR, report_filename)
    pdf_path = os.path.join(REPORTS_DIR, report_filename.replace(".md", ".pdf"))
    
    final_markdown_content = "".join(full_report)
    
    try:
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(final_markdown_content)
        print(f"Bitcoin report saved to: {report_path}")
        ...
        try:
            ...
            pdfkit.from_string(html_with_style, pdf_path, options=options)
            print(f"Bitcoin PDF version generated successfully at {pdf_path}")
        except Exception as e:
            print(f"\nCould not generate Bitcoin PDF. Please ensure 'wkhtmltopdf' is installed and in your system's PATH.")
            print(f"Error details: {e}")

    except IOError as e:
        print(f"Error writing Bitcoin report: {e}")
        return None
    
    return report_path

def _build_bitcoin_market_overview_section():
    """
    Builds a Bitcoin-focused market overview section.
    """
    try:
        # Get Bitcoin-specific analysis
        btc_signals_path = os.path.join(ANALYSIS_DIR, "signals_BTCUSDT.csv")
        btc_volatility_path = os.path.join(ANALYSIS_DIR, "volatility_BTCUSDT.csv")
        
        if not os.path.exists(btc_signals_path):
            return ["\n\n## Bitcoin Market Overview\n\n", "*Bitcoin analysis data not available.*\n\n"]
        
        # Get current Bitcoin state
        regime, persistence = _get_latest_regime_and_persistence(btc_signals_path)
        volatility_metrics = _get_latest_volatility_metrics(btc_volatility_path)
        
        overview_text = f"""\n\n## Bitcoin Market Overview

Bitcoin is currently in a **{regime}** phase, persisting for **{persistence} days**. """
        
        # Add Sharpe ratio context
        if volatility_metrics.get('sharpe') is not None:
            sharpe = volatility_metrics['sharpe']
            if sharpe > 2:
                sharpe_desc = "excellent"
            elif sharpe > 1:
                sharpe_desc = "good"
            else:
                sharpe_desc = "moderate"
            overview_text += f"Risk-adjusted returns show {sharpe_desc} performance with a 60-day Sharpe ratio of {sharpe:.2f}. "
        ...
        if not _is_ai_disabled():
            try:
                from src.reporters.ai_market_reporter_alt import generate_bitcoin_brief_opinion_alt
                ai_opinion = generate_bitcoin_brief_opinion_alt()
                if ai_opinion and ai_opinion.get('success', False):
                    overview_text += ai_opinion['analysis'] + "\n\n"
            except Exception as e:
                print(f"Could not generate AI overview for Bitcoin: {e}")
        
        return [overview_text]
        
    except Exception as e:
        print(f"Error building Bitcoin market overview: {e}")
        return ["## Bitcoin Market Overview\n\n", "*Error generating Bitcoin market overview.*\n\n"]
